...
class ChannelAttention_modified(tf.keras.layers.Layer):
    def __init__(self, in_planes, ratio=4, gamma = 2, b = 1):
        ...
        self.conv1 = tf.keras.layers.Conv2D(in_planes//ratio, kernel_size=kernel_size, strides=1, padding='same',
                                   kernel_regularizer=tf.keras.regularizers.l2(1e-4),
                                   use_bias=True, activation=tf.nn.relu)
        self.conv2 = tf.keras.layers.Conv2D(in_planes, kernel_size=kernel_size, strides=1, padding='same',
                                   kernel_regularizer=tf.keras.regularizers.l2(1e-4),
                                   use_bias=True)
