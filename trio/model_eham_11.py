...
class Clair3_P(tf.keras.Model):
    ...
    def __init__(self, add_indel_length=False, predict=False):
        ...
        self.L4 = tf.keras.layers.Dense(units=self.L4_pileup_num_units, activation='selu',
                                        kernel_regularizer=L2_regularizers)

        self.L4_dropout = tf.keras.layers.Dropout(rate=self.LSTM2_dropout_rate, seed=param.OPERATION_SEED)

        self.L5_1 = tf.keras.layers.Dense(units=self.L5_1_num_units, activation='selu',
                                          kernel_regularizer=L2_regularizers)

        self.L5_1_dropout = tf.keras.layers.Dropout(rate=self.L5_1_dropout_rate, seed=param.OPERATION_SEED)

        self.L5_2 = tf.keras.layers.Dense(units=self.L5_2_num_units, activation='selu',
                                          kernel_regularizer=L2_regularizers)
        ...
        if self.add_indel_length:
            self.L5_3 = tf.keras.layers.Dense(units=self.L5_3_num_units, activation='selu',
                                              kernel_regularizer=L2_regularizers)

            self.L5_3_dropout = tf.keras.layers.Dropout(rate=self.L5_3_dropout_rate, seed=param.OPERATION_SEED)

            self.L5_4 = tf.keras.layers.Dense(units=self.L5_4_num_units, activation='selu',
                                              kernel_regularizer=L2_regularizers)

            self.L5_4_dropout = tf.keras.layers.Dropout(rate=self.L5_4_dropout_rate, seed=param.OPERATION_SEED)

            self.Y_indel_length_logits_1 = tf.keras.layers.Dense(units=self.output_indel_length_shape_1,
                                                                 activation='selu', kernel_regularizer=L2_regularizers)

            self.Y_indel_length_logits_2 = tf.keras.layers.Dense(units=self.output_indel_length_shape_2,
                                                                 activation='selu', kernel_regularizer=L2_regularizers)
    ...
    def call(self, x, ):
        ...
        if self.add_indel_length:
            ...
            if self.predict:
                return tf.concat([y_gt21_logits, y_genotype_logits, y_indel_length_logits_1, y_indel_length_logits_2],
                                 axis=1)
...
############################### 改进的通道注意力机制 ###############################
class ChannelAttention_modified(tf.keras.layers.Layer):
    def __init__(self, in_planes, ratio=4, gamma=2, b=1):
        super(ChannelAttention_modified, self).__init__()
        self.avg = tf.keras.layers.GlobalAveragePooling2D()
        ...
        self.conv1 = tf.keras.layers.Conv1D(filters=1, kernel_size=kernel_size, padding='same', use_bias=False)
        self.conv2 = tf.keras.layers.Conv1D(filters=1, kernel_size=kernel_size, padding='same', use_bias=False)
...
class SpatialAttention_F(tf.keras.layers.Layer):
    def __init__(self, kernel_size=7):
        ...
        self.conv2 = regularized_padded_conv(1, kernel_size=kernel_size, strides=1, activation=tf.nn.sigmoid)
    ...
    def call(self, inputs):
        ...
        return out
############################### 空间注意力机制 ###############################
class SpatialAttention_modified(tf.keras.layers.Layer):
    def __init__(self, kernel_size=7):
        super(SpatialAttention_modified, self).__init__()
...
class SE_BLOCK(tf.keras.layers.Layer):
    def __init__(self, in_planes, ratio=4):
        ...
        self.conv1 = tf.keras.layers.Conv2D(in_planes // ratio, kernel_size=1, strides=1, padding='same',
                                            ...
                                            use_bias=True, activation=tf.nn.gelu)
...
class BasicBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, filter_num, stride=1, SeparableConv=False):
        ...
        ############################### ECA ###############################
        # self.eca = ECA_BLOCK(2 * filter_num)

        ############################### CBAM ###############################
        # self.ca = ChannelAttention_F(filter_num)
        # self.sa = SpatialAttention_F()
        ############################### SE ###############################
        # self.se = SE_BLOCK(filter_num)
        ############################### ECBAM ###############################
        # self.newatt = NewATT_BLOCK(2 * filter_num)
        # self.sa = SpatialAttention_F()
        ############################### modi_CBAM ###############################
        self.modi_CBAM = ChannelAttention_modified(filter_num)
        self.sa = SpatialAttention_modified()
...
class BasicConv2D(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size, strides, padding, SeparableConv=False):
        ...
        self.bn = tf.keras.layers.BatchNormalization()
        ############################### modi_CBAM ###############################
        self.modi_CBAM = ChannelAttention_modified(filters)
        self.sa = SpatialAttention_F()
        self.relu = tf.keras.layers.ReLU()
    ...
    def call(self, inputs):
        ...
        output = self.bn(output)
        ############################### modi_CBAM ###############################
        output = self.modi_CBAM(output) * output
...
class Clair3_F(tf.keras.Model):
    ...
    def __init__(self, add_indel_length=False, predict=False):
        ...
        self.L5_2 = tf.keras.layers.Dense(units=self.L5_1_num_units, activation='selu',
                                          kernel_regularizer=L2_regularizers)
...
class Clair3_Trio_Out3(tf.keras.Model):
    ...
    def __init__(self, add_indel_length=False, predict=False, is_padding=False, add_mcv_loss=False):
        ...
        self.c_Y_gt21_logits = tf.keras.layers.Dense(units=self.output_gt21_shape, activation='selu',
                                                     kernel_regularizer=L2_regularizers)
        self.c_Y_genotype_logits = tf.keras.layers.Dense(units=self.output_genotype_shape, activation='selu',
                                                         kernel_regularizer=L2_regularizers)
        self.c_Y_indel_length_logits_1 = tf.keras.layers.Dense(units=self.output_indel_length_shape_1,
                                                               activation='selu', kernel_regularizer=L2_regularizers)
        self.c_Y_indel_length_logits_2 = tf.keras.layers.Dense(units=self.output_indel_length_shape_2,
                                                               activation='selu', kernel_regularizer=L2_regularizers)

        self.p1_Y_gt21_logits = tf.keras.layers.Dense(units=self.output_gt21_shape, activation='selu',
                                                      kernel_regularizer=L2_regularizers)
        self.p1_Y_genotype_logits = tf.keras.layers.Dense(units=self.output_genotype_shape, activation='selu',
                                                          kernel_regularizer=L2_regularizers)
        self.p1_Y_indel_length_logits_1 = tf.keras.layers.Dense(units=self.output_indel_length_shape_1,
                                                                activation='selu', kernel_regularizer=L2_regularizers)
        self.p1_Y_indel_length_logits_2 = tf.keras.layers.Dense(units=self.output_indel_length_shape_2,
                                                                activation='selu', kernel_regularizer=L2_regularizers)

        self.p2_Y_gt21_logits = tf.keras.layers.Dense(units=self.output_gt21_shape, activation='selu',
                                                      kernel_regularizer=L2_regularizers)
        self.p2_Y_genotype_logits = tf.keras.layers.Dense(units=self.output_genotype_shape, activation='selu',
                                                          kernel_regularizer=L2_regularizers)
        self.p2_Y_indel_length_logits_1 = tf.keras.layers.Dense(units=self.output_indel_length_shape_1,
                                                                activation='selu', kernel_regularizer=L2_regularizers)
        self.p2_Y_indel_length_logits_2 = tf.keras.layers.Dense(units=self.output_indel_length_shape_2,
                                                                activation='selu', kernel_regularizer=L2_regularizers)
    ...
    def call(self, inputs):
        ...
        if self.predict:
            return tf.concat(
                [c_y_gt21_logits, c_y_genotype_logits, c_y_indel_length_logits_1, c_y_indel_length_logits_2, \
...
class Clair3_Trio_V_o1(tf.keras.Model):
    ...
    def __init__(self, add_indel_length=False, predict=False):
        ...
        self.c_L4 = tf.keras.layers.Dense(units=self.L4_num_units, activation='selu',
                                          kernel_regularizer=L2_regularizers)
        self.c_L4_dropout = tf.keras.layers.Dropout(rate=self.L4_dropout_rate, seed=param.OPERATION_SEED)

        self.p1_L4 = tf.keras.layers.Dense(units=self.L4_num_units, activation='selu',
                                           kernel_regularizer=L2_regularizers)
        self.p1_L4_dropout = tf.keras.layers.Dropout(rate=self.L4_dropout_rate, seed=param.OPERATION_SEED)

        self.p2_L4 = tf.keras.layers.Dense(units=self.L4_num_units, activation='selu',
                                           kernel_regularizer=L2_regularizers)
        ...
        self.Y_genotype_logits = tf.keras.layers.Dense(units=self.output_genotype_shape, activation='selu',
                                                       kernel_regularizer=L2_regularizers)
        self.Y_indel_length_logits_1 = tf.keras.layers.Dense(units=self.output_indel_length_shape_1, activation='selu',
                                                             kernel_regularizer=L2_regularizers)
        self.Y_indel_length_logits_2 = tf.keras.layers.Dense(units=self.output_indel_length_shape_2, activation='selu',
                                                             kernel_regularizer=L2_regularizers)
    ...
    def call(self, inputs):
        ...
        return [c_y_gt21_logits, c_y_genotype_logits, c_y_indel_length_logits_1, c_y_indel_length_logits_2, \
                ...
                tf.concat([c_y_gt21_logits, c_y_genotype_logits, c_y_indel_length_logits_1, c_y_indel_length_logits_2, \
                           p1_y_gt21_logits, p1_y_genotype_logits, p1_y_indel_length_logits_1,
                           p1_y_indel_length_logits_2, \
                           p2_y_gt21_logits, p2_y_genotype_logits, p2_y_indel_length_logits_1,
                           p2_y_indel_length_logits_2], axis=1)]
