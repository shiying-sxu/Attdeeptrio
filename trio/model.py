...
class SpatialPyramidPooling3D(tf.keras.layers.Layer):
    def __init__(self, pool_sizes=(2, 4, 8, 16)):
        super(SpatialPyramidPooling3D, self).__init__()
    ...
    def call(self, inputs):
        ...
        for pool_size in self.pool_sizes:
            pooled_output = tf.keras.layers.MaxPool3D(pool_size=(pool_size, pool_size, pool_size), padding='same')(inputs)
            pooled_outputs.append(pooled_output)

        # Flatten pooled outputs and concatenate
        output = tf.concat([tf.keras.layers.Flatten()(pooled_output) for pooled_output in pooled_outputs], axis=-1)
        return output
...
class BasicConv3D(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size, strides, padding, SeparableConv=False):
        super(BasicConv3D, self).__init__()
        conv = tf.keras.layers.SeparableConv3D if SeparableConv else tf.keras.layers.Conv3D
        self.conv = conv(filters=filters,
                                           kernel_size=kernel_size,
                                           strides=strides,
                                           padding=padding,
...
class ESAM(tf.keras.layers.Layer):
    def __init__(self, in_planes, ratio=4, gamma=2, b=1):
        super(ESAM, self).__init__()
        self.avg = tf.keras.layers.GlobalAveragePooling3D()
        self.max = tf.keras.layers.GlobalMaxPooling3D()
    ...
    def call(self, inputs):
        avg = self.avg(inputs)
        max = self.max(inputs)
        avg = tf.keras.layers.Reshape((1, 1, 1, avg.shape[1]))(avg)  # shape (None, 1, 1 feature)
        max = tf.keras.layers.Reshape((1, 1, 1, max.shape[1]))(max)  # shape (None, 1, 1 feature)
...
def regularized_padded_conv(*args, **kwargs):
    return tf.keras.layers.Conv3D(*args, **kwargs, padding='same', use_bias=False,
...
class BasicBlock_trio_noatt(tf.keras.layers.Layer):

    def __init__(self, filter_num, stride=1, SeparableConv=False):
        super(BasicBlock_trio_noatt, self).__init__()
        conv = tf.keras.layers.SeparableConv2D if SeparableConv else tf.keras.layers.Conv3D
...
class BasicBlock_trio(tf.keras.layers.Layer):

    def __init__(self, filter_num, stride=1, SeparableConv=False):
        super(BasicBlock_trio, self).__init__()
        conv = tf.keras.layers.SeparableConv2D if SeparableConv else tf.keras.layers.Conv3D
...
def make_basic_block_layer_trio(filter_num, blocks, stride=1, SeparableConv=False):
    res_block = tf.keras.Sequential()

    res_block.add(BasicBlock_trio(filter_num, stride=stride, SeparableConv=SeparableConv))

    for _ in range(1, blocks):
        res_block.add(BasicBlock_trio(filter_num, stride=1, SeparableConv=SeparableConv))
...
class SpatialAttention_F(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        avg_out = tf.reduce_mean(inputs, axis=4)
        max_out = tf.reduce_max(inputs, axis=4)
        out = tf.stack([avg_out, max_out], axis=4)
        out = self.conv2(self.conv1(out))

        return out

def make_basic_block_layer_trio_noatt(filter_num, blocks, stride=1, SeparableConv=False):
    res_block = tf.keras.Sequential()

    res_block.add(BasicBlock_trio_noatt(filter_num, stride=stride, SeparableConv=SeparableConv))

    for _ in range(1, blocks):
        res_block.add(BasicBlock_trio_noatt(filter_num, stride=1, SeparableConv=SeparableConv))
...
class Clair3_Trio_Out3(tf.keras.Model):
    ...
    def __init__(self, add_indel_length=False, predict=False, is_padding=False, add_mcv_loss=False):
        ...
        self.conv1 = BasicConv3D(filters=64,
                                 kernel_size=(3, 3, 3),
                                 strides=2,
                                 padding="same",)
        self.ESAM = ESAM(64)
        ...
        self.res_block1 = make_basic_block_layer_trio_noatt(filter_num=64,
                                            blocks=1, stride=1, SeparableConv=False)

        self.conv3 = BasicConv3D(filters=128,
                                 kernel_size=(3, 3, 3),
                                 strides=2,
                                 padding="same")

        self.res_block2 = make_basic_block_layer_trio_noatt(filter_num=128,
                                            blocks=1, stride=1, SeparableConv=False)

        self.conv5 = BasicConv3D(filters=256,
                                 kernel_size=(3, 3, 3),
                                 strides=2,
                                 padding="same")

        # self.res_block3 = make_basic_block_layer_trio(filter_num=256,
        #                                     blocks=1, stride=1)
        self.conv7 = BasicConv3D(filters=512,
                                 kernel_size=(3, 3, 3),
                                 strides=2,
                                 padding="same")


        self.pyramidpolling = SpatialPyramidPooling3D()
        self.L3_dropout = tf.keras.layers.Dropout(rate=self.L3_dropout_rate)
